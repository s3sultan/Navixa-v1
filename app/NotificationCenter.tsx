...
export default function NotificationCenter({onPreviewReminder}:Props){
  ...
  const saveTelegram=(event:FormEvent)=>{event.preventDefault();const next={token:config.token.trim(),chatId:config.chatId.trim()};if(!next.token||!next.chatId){setStatus("أدخل Bot Token وChat ID أولًا");return}setTelegramConfig(next);setConfig(next);setConnected(true);setStatus("تم تفعيل Telegram لهذه الجلسة فقط؛ لن تُحفظ بيانات البوت على جهازك")};
  const disconnect=()=>{clearTelegramConfig();setConfig({token:"",chatId:""});setConnected(false);setStatus("تم فصل Telegram وحذف بياناته من ذاكرة الجلسة")};
  ...
  return <div className="notification-center">
    ...
    {!connected&&<form className="telegram-connect" onSubmit={saveTelegram}><p>اربط بوتك الشخصي لاستقبال تنبيهاتك أنت فقط. بيانات البوت تستخدم في هذه الجلسة ولا تحفظ في المتصفح.</p><input type="password" autoComplete="off" value={config.token} onChange={e=>setConfig({...config,token:e.target.value})} placeholder="Bot Token" required/><input autoComplete="off" value={config.chatId} onChange={e=>setConfig({...config,chatId:e.target.value})} placeholder="Chat ID" required/><button>تفعيل Telegram للجلسة</button><small>يمكنك أخذ الرمز من BotFather ومعرّف المحادثة من userinfobot. بعد إعادة تحميل الصفحة ستحتاج إلى إدخالهما من جديد.</small></form>}
