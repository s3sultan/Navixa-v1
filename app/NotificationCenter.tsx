...
import {ensureNavixaPushSubscription,sendNavixaPushTest,showNavixaDeviceNotification,syncExistingNavixaPushSubscription} from "./pushClient";
import PushCategoryControls from "./PushCategoryControls";
...
const REMINDER_STREAMS=[
  ...
  ...(HEALTH_FEATURE_ENABLED?[{icon:"◎",title:"تركيزك وصحتك",detail:"الماء، الحركة، راحة العين وجلسات التركيز.",href:"/#focus"}]:[]),
...
export default function NotificationCenter({onPreviewReminder}:Props){
  ...
  useEffect(()=>{
    ...
    setReminderPrefs(getPersonalReminderPrefs());
    setBrowserPermission("Notification" in window?Notification.permission:"unsupported");
    void loadTelegram();
    void syncExistingNavixaPushSubscription().then(subscription=>{
      if(!subscription)return;
      setBrowserPermission("granted");
      const current=getPersonalReminderPrefs();
      if(!current.browser){const next={...current,browser:true};setReminderPrefs(next);setPersonalReminderPrefs(next)}
    });
  },[]);
  ...
  const requestBrowserNotifications=async()=>{
    ...
    try{
      ...
      setBrowserPermission("Notification" in window?Notification.permission:"unsupported");
      if(!subscription){setStatus("اضغط التفعيل مرة أخرى بعد السماح بالتنبيهات.");return}
      updateReminderPrefs({browser:true});
      try{
        await sendNavixaPushTest(subscription.endpoint);
        setStatus(subscription.accountBound?"تم تفعيل Push وربطه بحسابك، ووصل اختبار حقيقي من الخادم إلى الجهاز 🔔":"تم تفعيل Push على هذا الجهاز، ووصل اختبار حقيقي من الخادم 🔔");
      }catch{
        setStatus("تم تسجيل الجهاز في Push، لكن اختبار الإرسال لم يكتمل. سيُعاد مزامنة الاشتراك تلقائيًا عند فتح NAVIXA.");
      }
    }catch(error){setBrowserPermission("Notification" in window?Notification.permission:"unsupported");setStatus(error instanceof Error?error.message:"تعذر تفعيل Push على هذا الجهاز.")}
    finally{setTesting(false)}
  };
  const testBrowserPush=async()=>{
    setTesting(true);setStatus("");
    try{
      const subscription=await ensureNavixaPushSubscription({requestPermission:false});
      if(!subscription){setStatus("Push غير مسجل على هذا الجهاز. فعّله أولًا.");return}
      await sendNavixaPushTest(subscription.endpoint);
      setBrowserPermission("granted");
      setStatus(subscription.accountBound?"✅ اختبار Push الحقيقي وصل من خادم NAVIXA لهذا الجهاز والحساب.":"✅ اختبار Push الحقيقي أُرسل لهذا الجهاز بنجاح.");
    }catch(error){setStatus(error instanceof Error?error.message:"تعذر اختبار Push الآن.")}
    finally{setTesting(false)}
  };
  const previewReminder=async()=>{const title="تذكير ماء لطيف",body="مر وقت منذ آخر كوب ماء مسجّل. خذ رشفة إذا احتجت.";if(reminderPrefs?.browser)await showNavixaDeviceNotification(title,{body,tag:"navixa-preview",url:"/"}).catch(()=>false);onPreviewReminder(`${title} — ${body}`);setStatus("ظهرت معاينة التذكير الآن")};
  ...
  return <div className="notification-center">
    ...
    <div className="alert-prefs-list">{prefs&&ALERT_TYPES.filter(type=>HEALTH_FEATURE_ENABLED||!isHealthAlertType(type)).map(type=>{const screenManaged=type==="screen";return <div key={type} className="alert-prefs-row"><b>{ALERT_LABELS[type]}</b><label className={screenManaged?"unavailable":""}><input type="checkbox" checked={screenManaged?true:prefs[type].screen} disabled={screenManaged} onChange={()=>toggle(type,"screen")}/>{screenManaged?" داخل أداة الشاشة":" داخل NAVIXA"}</label><label className={!connected?"unavailable":""}><input type="checkbox" checked={prefs[type].telegram} disabled={!connected} onChange={()=>toggle(type,"telegram")}/> Telegram</label></div>})}</div>
  </div>;
}
