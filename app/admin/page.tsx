...
export default function AdminPage() {
  ...
  return <main dir="rtl" className="admin-shell">
    ...
    <aside className="admin-side"><div className="logo"><span>ن</span><div><b>NAVIXA</b><small>ADMIN CENTER</small></div></div><div className="admin-badge">لوحة الإدارة</div>
      <nav>{["نظرة عامة","المميزات","الزوار","الأجهزة","الأتمتة"].map((x,i)=><button key={x} className={section===x?"on":""} onClick={()=>{setSection(x);document.getElementById(x==="الزوار"?"visitors":x==="الأجهزة"?"devices":x==="الأتمتة"?"automation-admin":"overview")?.scrollIntoView({behavior:"smooth"})}}><i>{["⌂","✦","♙","⌁","⚙"][i]}</i>{x}</button>)}<button onClick={()=>window.location.href="/admin/ads"}><i>▣</i>الإعلانات<em>4</em></button><button onClick={()=>window.location.href="/emergency"}><i>⚠</i>الطوارئ</button>{["التنبيهات","سجل النظام"].map((x,i)=><button key={x} className={section===x?"on":""} onClick={()=>{setSection(x);tell(x==="التنبيهات"?"3 تنبيهات: اجتماع قريب، أجهزة ضعيفة، ومهمة متأخرة":`تم فتح ${x}`)}}><i>{["!","▤"][i]}</i>{x}{x==="التنبيهات"&&<em>3</em>}</button>)}</nav>
