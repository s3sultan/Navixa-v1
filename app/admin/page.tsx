...
export default function AdminPage(){
  ...
  return <main dir="rtl" className="admin-shell">{toast&&<div className="toast">✓ {toast}</div>}
    ...
    <section className="admin-page"><header className="admin-header" id="overview"><div><small>مركز التحكم الكامل</small><h1>لوحة إدارة NAVIXA</h1><p>كل المستخدمين والميزات والمحتوى والصلاحيات من مكان واحد.</p></div><div><button onClick={()=>tell("لا توجد أعطال حرجة")}>♢<i/></button><Link href="/">عرض التطبيق ↗</Link></div></header>
      ...
      <section className="settings-group" id="settings"><div className="section-label"><small>الإعدادات</small><h2>تفضيلات النظام</h2></div><div className="settings-stack"><AdminHealthSettings/><AdminCounterSettings/><AdminAssistantSettings/><AdminAssistantLearningSettings/><AdminMeetingSettings/><AdminUserAuthSettings/><AdminSubscriptionSettings/><AdminReferralSettings/><AdminAlertSettings/><AdminMatchSettings/></div></section>
