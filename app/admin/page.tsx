...
import AdminAccessAuditPanel from "./AdminAccessAuditPanel";
...
import AdminPushLab from "./settings/AdminPushLab";
...
import AdminPlanPricing from "./settings/AdminPlanPricing";
...
const navGroups = [
  ...
  { title: "المتابعة", items: [["سجل النشاط", "activity", "≡"], ["الأداء", "performance", "↗"], ["الصحة", "site-health", "✓"], ["الخريطة", "heatmap", "▦"]] },
] as const;
...
export default function AdminPage() {
  ...
  return <main dir="rtl" className="admin-shell dashboard-v2">
    ...
    <section className="admin-page">
      ...
      <section className="admin-section admin-fold" id="permissions"><details className="admin-fold-card"><summary><span>الوصول والصلاحيات</span><small>صلاحيات موثقة من جلسة الخادم</small></summary><div className="admin-fold-content"><AdminAccessAuditPanel mode="permissions" /></div></details></section>

      <section className="admin-section admin-fold" id="content"><details className="admin-fold-card"><summary><span>مركز العمليات</span><small>ابدأ من الأداة المتصلة المناسبة</small></summary><div className="admin-fold-content"><div className="operations-grid"><section className="operations-card"><small>الحسابات والتنبيهات</small><h2>إعداد الدخول والتذكيرات</h2><p>اضبط حسابات المستخدمين وقنوات التنبيه من الأقسام المتصلة، دون زر إجراءات شكلية.</p><div className="admin-actions"><button type="button" onClick={() => go("الإعدادات", "settings")}>فتح الإعدادات</button><button type="button" onClick={() => go("سجل النشاط", "activity")}>عرض السجل والاستخدام</button></div></section><section className="operations-card" id="integrations"><small>الحالة التجارية</small><h2>الدفع والتكاملات</h2><p>الدفع العام يبقى مقفلًا حتى اكتمال اعتماد المزود واختبار الربط، بينما قائمة الأسعار تبقى معلنة وواضحة.</p><div className="admin-actions"><button type="button" onClick={() => go("الاشتراكات", "subscriptions")}>فتح الاشتراكات</button><button type="button" onClick={() => { window.location.href = "/admin/support"; }}>الدعم الموحد</button></div></section></div></div></details></section>

      <section className="admin-section" id="settings"><div className="section-label"><small>إعدادات المنصة</small><h2>الإعدادات المنظمة</h2><p>افتح ما تحتاجه فقط لتبقى الإدارة هادئة وسهلة.</p></div><div className="settings-stack"><details className="settings-group"><summary>الحماية والدومين <span>الدخول · البريد · انتهاء النطاق</span></summary><div className="settings-group-content"><AdminDomainExpiryAlert /><AdminServiceTransitionSettings /><AdminUserAuthSettings /></div></details><details className="settings-group"><summary>التنبيهات والمباريات <span>القنوات · القواعد · Push · العرض</span></summary><div className="settings-group-content"><AdminAlertSettings /><AdminPushLab /><AdminMatchSettings /></div></details><details className="settings-group"><summary>المساعد والاجتماعات <span>الردود · التعلم · التلخيص</span></summary><div className="settings-group-content"><AdminAssistantSettings /><AdminAssistantLearningSettings /><AdminMeetingSettings /></div></details><details className="settings-group"><summary>الصحة والمحتوى <span>العافية · العدادات · التمارين</span></summary><div className="settings-group-content"><AdminHealthSettings /><AdminCounterSettings /></div></details><details className="settings-group" id="subscriptions"><summary>الاشتراكات والأسعار <span>هِمّة · عَزْم · التجربة · الإحالات · الخصومات</span></summary><div className="settings-group-content"><AdminPlanPricing /><AdminSubscriptionSettings /><AdminReferralSettings /><AdminDiscountCodes /></div></details></div></section>
      <section className="admin-section admin-fold" id="activity"><details className="admin-fold-card" open><summary><span>سجل النشاط والاستخدام</span><small>إجراءات إدارية موثقة وبيانات استخدام مجمعة</small></summary><div className="admin-fold-content"><AdminAccessAuditPanel mode="activity" /><AdminUsageAnalytics /></div></details></section>
