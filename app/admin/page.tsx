...
import "./admin.css";
import AdminHealthSettings from "./AdminHealthSettings";
import AdminCounterSettings from "./AdminCounterSettings";
import AdminAssistantSettings from "./AdminAssistantSettings";
...
export default function AdminPage(){
  ...
  return <main dir="rtl" className="admin-shell">{toast&&<div className="toast">✓ {toast}</div>}
    <aside className="admin-side"><div className="logo"><span>ن</span><div><b>NAVIXA</b><small>ADMIN CENTER</small></div></div><div className="admin-badge">صلاحيات المدير الكاملة</div><nav>
      {[["نظرة عامة","overview","⌂"],["المميزات","features","✦"],["المستخدمون","users","♙"],["الصلاحيات","permissions","⌘"],["المحتوى","content","▤"],["التكاملات","integrations","⌁"],["الإعدادات","settings","⚙"]].map(x=><button key={x[0]} className={active===x[0]?"on":""} onClick={()=>go(x[0],x[1])}><i>{x[2]}</i>{x[0]}</button>)}
      <button onClick={()=>location.href="/admin/ads"}><i>▣</i>الإعلانات</button><button onClick={()=>location.href="/admin/emergency"}><i>⚠</i>الطوارئ</button><button onClick={()=>location.href="/admin/social"}><i>◎</i>التواصل</button></nav><div className="admin-side-bottom"><a href="/">← العودة إلى NAVIXA</a><div><span>س</span><p><b>سلطان</b><small>المدير الأعلى</small></p></div></div></aside>
    <section className="admin-page"><header className="admin-header" id="overview"><div><small>مركز التحكم الكامل</small><h1>لوحة إدارة NAVIXA</h1><p>كل المستخدمين والميزات والمحتوى والصلاحيات من مكان واحد.</p></div><div><button onClick={()=>tell("لا توجد أعطال حرجة")}>♢<i/></button><a href="/">عرض التطبيق ↗</a></div></header>
