...
export default function AdminLogin(){
  ...
  return <main className="login-page" dir="rtl"><section className="login-card"><Link href="/" className="login-brand"><span className="login-logo-mark"><img src="/navixa-mark.png" alt="" /></span><div><b>NAVIXA</b><small>ADMIN CENTER</small></div></Link><div className="login-title"><small>دخول الإدارة الآمن</small><h1>مرحبًا بعودتك</h1><p>تفضل بالدخول، ثم اختر حساب Google المصرّح له بالوصول إلى لوحة الإدارة.</p></div><div ref={swipeTrack} className={`swipe-login ${unlocked?"unlocked":""}`} onPointerMove={e=>{if(e.currentTarget.hasPointerCapture(e.pointerId))moveSwipe(e.clientX)}} onPointerUp={e=>{e.currentTarget.releasePointerCapture(e.pointerId);if(!unlocked)setSwipe(0)}} onPointerCancel={()=>!unlocked&&setSwipe(0)}><span>{unlocked?"تم التحقق — اختر حسابك":"تفضل بالدخول"}</span><button aria-label="اسحب شعار NAVIXA للدخول" style={{right:`calc(5px + ${swipe*100}% - ${swipe*62}px)`}} onPointerDown={e=>{e.preventDefault();e.currentTarget.parentElement?.setPointerCapture(e.pointerId)}}><img src="/navixa-mark.png" alt="شعار NAVIXA" /></button></div><div className={`google-login-box ${unlocked?"shown":""}`}><div ref={googleButton}/>{loading&&unlocked&&<span>جارٍ تجهيز تسجيل الدخول…</span>}</div><p className="oauth-origin-hint">إذا ظهر خطأ <b>origin_mismatch</b>، يجب أن يضيف مالك التطبيق النطاق <code>https://navixa.s2shug.workers.dev</code> إلى Authorized JavaScript origins في Google Cloud Console.</p>{error&&<p className="login-error">{error}</p>}<p className="allowed-email">الحساب المسموح: <b>s2shug@gmail.com</b></p><p className="privacy">🔒 NAVIXA لا يرى كلمة مرور Google ولا يحفظها.</p></section><aside><div className="admin-aside-mark"><img src="/navixa-mark.png" alt="شعار NAVIXA" /></div><h2>إدارة NAVIXA<br/>بوضوح وثقة.</h2><p>تفضل بالدخول، ثم دع Google تؤكد الحساب قبل فتح لوحة الإدارة.</p><small>حماية واضحة · صلاحيات محددة · خصوصية أولًا</small></aside></main>
}
