...
export default function AdsPage(){
  ...
  return <main dir="rtl" className="ads-shell">
    ...
    {showCreate&&<div className="modal-back" onClick={()=>setShowCreate(false)}><form className="ad-modal" onClick={e=>e.stopPropagation()} onSubmit={e=>{e.preventDefault();setShowCreate(false);say("تم إنشاء الحملة وحفظها كمسودة")}}><button type="button" className="close" onClick={()=>setShowCreate(false)}>×</button><small>حملة جديدة</small><h2>أنشئ إعلانًا جديدًا</h2><label>اسم الحملة<input required placeholder="مثال: عروض موسم الحصاد"/></label><label>المعلن<input required placeholder="اسم الشركة أو الجهة"/></label><label>مكان الظهور<select><option>الشريط الرئيسي</option><option>داخل المزرعة</option><option>مساعد NAVIXA</option><option>صفحة التقارير</option></select></label><div><label>الميزانية<input type="number" placeholder="2,000"/></label><label>تاريخ البداية<input type="date"/></label></div><button type="submit">إنشاء الحملة</button></form></div>}
  </main>
}
