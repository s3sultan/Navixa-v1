...
export default function AdminUserAuthSettings() {
  ...
  const sendTelegramTest = async () => {
    setTelegramTestBusy(true); setNotice("");
    const response = await fetch("/api/admin/telegram-test", { method: "POST" });
    const data = await response.json().catch(() => ({})) as { message?: string; error?: string };
    setTelegramTestBusy(false); setNotice(response.ok ? (data.message || "تم إرسال اختبار Telegram") : (data.error || "تعذر إرسال اختبار Telegram"));
  };
