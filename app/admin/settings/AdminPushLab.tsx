...
export default function AdminPushLab(){
  ...
  return <section className="panel alert-settings-panel admin-push-lab">
    <div className="panel-head push-lab-head"><div><small>مختبر Push</small><h2>تجربة شكل ونص التنبيه</h2></div><button type="button" onClick={send} disabled={busy}>{busy?"جاري الإرسال":"إرسال تجربة"}</button></div>
    ...
    <div className="operations-grid push-lab-grid">
      <div className="operations-card push-lab-editor">
        ...
        <div className="push-lab-controls">
          ...
          <label className="admin-field range-field"><small>وقت المعاينة: {previewSeconds} ث</small><input type="range" min="3" max="30" value={previewSeconds} onChange={e=>setPreviewSeconds(Number(e.target.value))}/></label>
        </div>
      ...
      <div className="operations-card push-lab-preview"><small>معاينة NAVIXA</small><div style={{...previewStyle,padding:"14px",borderRadius:"14px",background:"var(--panel,#fff)",marginTop:"10px"}}><b>{title||"NAVIXA"}</b><p>{body||"لديك تنبيه جديد"}</p><small>سيختفي من هذه المعاينة بعد {previewSeconds} ثانية. إشعار النظام نفسه يتحكم به الجهاز.</small></div></div>
    </div>
  </section>;
}
