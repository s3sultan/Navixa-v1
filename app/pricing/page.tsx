...
export default function PricingPage(){
  ...
  return <main dir="rtl" style={{minHeight:"100vh",padding:"28px 0",background:"#f7f8fa"}}>
    <section className="public-pricing" aria-labelledby="pricing-title">
      <div className="public-pricing-head"><div><small>NAVIXA SA · قائمة الأسعار الرسمية</small><h1 id="pricing-title" style={{margin:0,fontSize:"clamp(30px,3vw,46px)"}}>الأسعار</h1><p>أسعار خدمات NAVIXA معلنة بوضوح بالريال السعودي. المبلغ النهائي يظهر للمستخدم قبل تأكيد عملية الدفع.</p></div><Link href="/">العودة للرئيسية ←</Link></div>
      <div className="public-pricing-grid">{plans.map(plan=><article key={plan.id} className={`public-price-card ${plan.id}`}><div><span className="public-plan-name">{plan.name}</span><small>{plan.periodLabel}</small></div><p><strong>{riyals(plan.amount)}</strong><span>ريال</span></p><Link href={plan.id==="monthly"?"/plus":"/sprint"}>تفاصيل الباقة ←</Link></article>)}</div>
