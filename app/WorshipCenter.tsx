...
export default function WorshipCenter(){
  ...
  const [prayersDone,setPrayersDone]=useState<TrackedPrayer[]>([]);
  ...
  const togglePrayerDone=(name:TrackedPrayer)=>{setPrayersDone(current=>{const next=current.includes(name)?current.filter(item=>item!==name):[...current,name];localStorage.setItem(prayerTrackingKey(today()),JSON.stringify(next));return next})};
