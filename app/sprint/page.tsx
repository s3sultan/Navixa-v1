...
export default function SprintPage(){return <main className="plus-page plus-simple" dir={languageIdentity.ar.direction}>
  ...
  <section className="plus-confidence" id="sprint-details" aria-labelledby="sprint-details-title"><div className="plus-confidence-copy"><small>المدة والسعر</small><h2 id="sprint-details-title"><PlanPriceInline plan="sprint" /> مقابل خمسة أيام من العَزْم.</h2><p>مناسب لدورة قصيرة، أسبوع اختبارات، مؤتمر أو سلسلة اجتماعات مكثفة. تبدأ المدة بعد تفعيل الاشتراك، وعَزْم باقة محددة المدة وليست اشتراكًا شهريًا.</p></div><div className="plus-confidence-grid">{trust.map(([title,description])=><article key={title}><b>{title}</b><p>{description}</p></article>)}</div></section>
  ...
  <section className="plus-promise"><div><small>عَزْم أو هِمّة؟</small><h2>ابدأ بعَزْم، وكمل بهِمّة.</h2><p><strong>عَزْم:</strong> خمسة أيام بسعر <PlanPriceInline plan="sprint" /> لهدف قصير ومكثف. <strong>هِمّة:</strong> شهر كامل بسعر <PlanPriceInline plan="monthly" /> للاستمرار والمتابعة على المدى الأطول.</p></div><Link href="/plus">شاهد تفاصيل هِمّة ←</Link></section>
