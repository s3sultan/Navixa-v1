...
export default function Home(){
  ...
  useEffect(()=>{if(!running)return;const timer=setInterval(()=>setSeconds(s=>{if(s<=1){setRunning(false);notify("أحسنت! انتهت جلسة التركيز");notifyTelegram("🎯 تذكير NAVIXA: انتهت جلسة تركيز (25 دقيقة)");logSession();return 25*60}return s-1}),1000);return()=>clearInterval(timer)},[running]);
