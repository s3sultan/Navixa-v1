...
export default function Home(){
  ...
  const toggleScreen=async()=>{
    if(screen){screenRef.current?.getTracks().forEach(t=>t.stop());screenRef.current=null;if(screenVideoRef.current)screenVideoRef.current.srcObject=null;setScreen(false);return}
    try{const stream=await navigator.mediaDevices.getDisplayMedia({video:true,audio:false});screenRef.current=stream;stream.getVideoTracks()[0].onended=()=>{setScreen(false);screenRef.current=null};setScreen(true);notify("تمت المشاركة — ارسم إطار المتابعة داخل المعاينة")}
