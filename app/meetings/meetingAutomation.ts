...
import { listPersistentAutomationRuns, persistAutomationRunBestEffort } from "../automationRunHistoryClient.ts";
...
export async function listMeetingSummaryAutomationRuns(): Promise<AutomationRun[]> {
  const [localRuns, persistentRuns] = await Promise.all([
    ...
    listPersistentAutomationRuns({ skillId: MEETING_SUMMARY_SKILL_ID, limit: 50 }),
  ]);
