...
export default function AccountAccess() {
  ...
  const [emergencyTelegram, setEmergencyTelegram] = useState(false);
  ...
  const loadTelegram = async () => { const response = await fetch("/api/account/telegram/link", { cache: "no-store" }); const data = await response.json().catch(() => ({ enabled: false, linked: false })); const state = response.ok ? data as TelegramLink : { enabled: false, linked: false }; setTelegram(state); if (state.linked) { const preference = await fetch("/api/account/telegram/preferences", { cache: "no-store" }).then(value => value.json()).catch(() => ({ enabled: false, preferences: {} })); setRenewalTelegram(Boolean(preference.preferences?.renewal ?? preference.enabled)); setEmergencyTelegram(Boolean(preference.preferences?.emergency)); } else { setRenewalTelegram(false); setEmergencyTelegram(false); } };
  ...
  useEffect(() => {
    ...
    const mount = () => {
      ...
      google.accounts.id.initialize({ client_id: GOOGLE_CLIENT_ID, callback: ({ credential }) => { if (active) void finishGoogleLogin(credential); }, itp_support: true, auto_select: false, cancel_on_tap_outside: true });
    ...
    if (!script) { script = document.createElement("script"); script.src = "https://accounts.google.com/gsi/client"; script.async = true; script.dataset.navixaGoogle = "true"; script.addEventListener("error", failed, { once: true }); document.head.appendChild(script); }
    else script.addEventListener("error", failed, { once: true });
    let attempts = 0;
    const retryGoogleMount = () => {
      if (!active || mount()) return;
      attempts += 1;
      if (attempts >= 67) { setGoogleState("error"); return; }
      timeout = setTimeout(retryGoogleMount, 180);
    };
    retryGoogleMount();
    return () => { active = false; if (timeout) clearTimeout(timeout); script?.removeEventListener("error", failed); };
  ...
  const setEmergencyReminder = async (enabled: boolean) => { setBusy(true); setNotice(""); const response = await fetch("/api/account/telegram/preferences", { method: "POST", headers, body: JSON.stringify({ type: "emergency", enabled }) }); const data = await response.json().catch(() => ({})); setBusy(false); if (!response.ok) { setNotice(data.error || "تعذر تحديث تنبيه الطوارئ"); return; } setEmergencyTelegram(enabled); setNotice(enabled ? "تم تفعيل تنبيهات الطوارئ عبر Telegram." : "تم إيقاف تنبيهات الطوارئ عبر Telegram."); };
  ...
  if (!session.enabled) return <section className="account-card account-closed"><span>التجربة المبكرة</span><h2>قريبًا</h2><p>حسابات NAVIXA هِمّة ستفتح تدريجيًا بعد تهيئة بريد الدخول الآمن. ميزاتك المحلية لا تحتاج حسابًا ولا تتأثر بهذا الانتظار.</p><a href="/plus">سجّل اهتمامك في هِمّة</a></section>;
  if (session.signedIn && session.user) { const plusEnd = session.plus?.status === "trial" ? session.plus.trial_ends_at : session.plus?.subscription_ends_at; const plusText = session.plus?.status === "trial" ? "تجربة هِمّة نشطة" : session.plus?.status === "active" ? "اشتراك هِمّة نشط" : session.earlyAccessEnabled ? "ستظهر تجربة هِمّة هنا بعد التحقق" : "سيظهر اشتراكك هنا عند تفعيله"; return <section className="account-card account-signed"><span>تم الدخول</span><h2>مرحبًا بك في NAVIXA</h2><p>{session.user.email}</p><div className="account-status"><b>{plusText}</b><small>{plusEnd ? `تنتهي في ${new Intl.DateTimeFormat("ar-SA", { day: "numeric", month: "long", year: "numeric" }).format(new Date(plusEnd))}` : session.passkeysEnabled ? "فعّل الدخول السريع ببصمة الجهاز أو Face ID. سيبقى رمز البريد متاحًا لاسترداد الوصول." : "يمكنك متابعة حسابك من هذا الجهاز بأمان."}</small></div>{session.passkeysEnabled && <button className="account-passkey" disabled={busy} onClick={() => void enablePasskey()}>{busy ? "جارٍ التفعيل…" : "فعّل الدخول السريع لهذا الجهاز"}</button>}{telegram?.enabled && <div className="account-telegram"><b>تنبيهات Telegram</b><small>{telegram.linked ? "البوت مرتبط بحسابك. تصلك فقط التنبيهات التي تختارها." : "اربط بوت NAVIXA مرة واحدة؛ لا نطلب منك Bot Token أو Chat ID."}</small><button className="account-secondary" disabled={busy} onClick={() => void (telegram.linked ? unlinkTelegram() : linkTelegram())}>{busy ? "جارٍ التنفيذ…" : telegram.linked ? "فصل Telegram" : "ربط Telegram"}</button>{telegram.linked && <><button className="account-secondary" disabled={busy} onClick={() => void setRenewalReminder(!renewalTelegram)}>{renewalTelegram ? "إيقاف تذكير التجديد" : "فعّل تذكير التجديد"}</button><button className="account-secondary" disabled={busy} onClick={() => void setEmergencyReminder(!emergencyTelegram)}>{emergencyTelegram ? "إيقاف تنبيهات الطوارئ" : "فعّل تنبيهات الطوارئ"}</button></>}</div>}<button disabled={busy} onClick={() => void logout()}>{busy ? "جارٍ الخروج…" : "تسجيل الخروج"}</button>{notice && <p className="account-notice" role="status">{notice}</p>}</section>; }
  return <section className="account-card"><span>حساب NAVIXA</span><h2>{sent ? "أدخل الرمز" : isRememberedEmail ? "مرحبًا بعودتك" : "ادخل ببريدك"}</h2><p>{sent ? "أرسلنا رمزًا من ستة أرقام، صالحًا لمدة 10 دقائق ويُستخدم مرة واحدة فقط." : isRememberedEmail ? "بريدك ظاهر على هذا الجهاز. إذا انتهت الجلسة، أرسل رمز دخول جديدًا فقط." : "لا تحتاج إلى كلمة مرور. يمكنك استخدام بريدك أو حساب Google الموثق."}</p>{!sent ? <>{session.googleLoginEnabled && <div className="account-google"><div ref={googleButton} aria-label="الدخول بحساب Google" /><small>{googleState === "loading" ? "جارٍ تجهيز زر Google…" : googleState === "error" ? "تعذر تجهيز Google الآن؛ استخدم رمز البريد مؤقتًا." : "لا نرى كلمة مرور Google ولا نحفظها."}</small></div>}<div className="account-divider" aria-hidden="true">أو</div><label>البريد الإلكتروني<input type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" autoComplete="email" maxLength={160}/></label>{isRememberedEmail && <button type="button" className="account-link" disabled={busy} onClick={forgetRememberedEmail}>ليس بريدي أو استخدم بريدًا آخر</button>}<button disabled={busy || !email.includes("@") } onClick={() => void requestCode()}>{busy ? "جارٍ الإرسال…" : isRememberedEmail ? "أرسل رمز الدخول إلى بريدي" : "أرسل رمز الدخول"}</button>{session.passkeysEnabled && <button className="account-secondary" disabled={busy || !email.includes("@") } onClick={() => void loginWithPasskey()}>الدخول السريع ببصمة الجهاز</button>}</> : <><label>رمز الدخول<input type="text" name="one-time-code" inputMode="numeric" pattern="[0-9]*" autoComplete="one-time-code" enterKeyHint="done" value={code} onChange={event => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="000000" maxLength={6}/></label><button disabled={busy || code.length !== 6} onClick={() => void verify()}>{busy ? "جارٍ التحقق…" : "تأكيد الدخول"}</button><button className="account-link" disabled={busy} onClick={() => { setSent(false); setCode(""); }}>تغيير البريد</button></>}{notice && <p className="account-notice" role="status">{notice}</p>}</section>;
}
