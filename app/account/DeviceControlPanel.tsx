...
export default function DeviceControlPanel(){
  ...
  const load=useCallback(async()=>{try{const r=await fetch("/api/device-control",{cache:"no-store"});const p=await r.json() as Payload;if(!r.ok){setData({error:p.error||"تعذر قراءة حالة الأجهزة"});throw new Error(p.error||"تعذر قراءة حالة الأجهزة")}setData(p)}catch(e){setNotice(e instanceof Error?e.message:"تعذر قراءة حالة الأجهزة")}finally{setLoading(false)}},[]);
  useEffect(()=>{void load();const timer=window.setInterval(()=>void load(),15000);return()=>window.clearInterval(timer)},[load]);
  const send=async(command:Command)=>{setSending(command);setNotice("");try{const r=await fetch("/api/device-control",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"create",command})});const p=await r.json() as {error?:string;reused?:boolean;pushDelivered?:number};if(!r.ok)throw new Error(p.error||"تعذر إرسال الطلب");await load();setNotice(p.reused?"هذا الطلب ما زال بانتظار الكمبيوتر، لذلك لم نكرر إرساله":Number(p.pushDelivered)>0?"تم إرسال الطلب، ووصل Push إلى الكمبيوتر النشط 🔔":"تم إرسال الطلب. إذا لم يكن Push مفعّلًا على الكمبيوتر فسيظهر عند فتح NAVIXA") }catch(e){setNotice(e instanceof Error?e.message:"تعذر إرسال الطلب")}finally{setSending(null)}};
