...
type Recent={id:string;command:Command;status:"pending"|"acknowledged"|"dismissed";created_at:string;expires_at:string};
...
const statusLabel=(value:Recent["status"])=>value==="pending"?"بانتظار الكمبيوتر":value==="acknowledged"?"تمت الاستجابة":"تم التجاهل";
...
export default function DeviceControlPanel(){
  ...
  const load=useCallback(async()=>{try{const r=await fetch("/api/device-control",{cache:"no-store"});const p=await r.json() as Payload;if(!r.ok)throw new Error(p.error||"تعذر قراءة حالة الأجهزة");setData(p)}catch(e){setNotice(e instanceof Error?e.message:"تعذر قراءة حالة الأجهزة")}finally{setLoading(false)}},[]);
  useEffect(()=>{void load();const timer=window.setInterval(()=>void load(),15000);return()=>window.clearInterval(timer)},[load]);
  const send=async(command:Command)=>{setSending(command);setNotice("");try{const r=await fetch("/api/device-control",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"create",command})});const p=await r.json() as {error?:string};if(!r.ok)throw new Error(p.error||"تعذر إرسال الطلب");setNotice("تم إرسال الطلب للكمبيوتر لمدة 5 دقائق فقط");await load()}catch(e){setNotice(e instanceof Error?e.message:"تعذر إرسال الطلب")}finally{setSending(null)}};
