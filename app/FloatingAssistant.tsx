...
export default function FloatingAssistant({ onAddTask, openRequest }: { onAddTask: (title: string) => void; openRequest: number }) {
  ...
  const understand = (raw: string) => {
    ...
    if (/(?:رتب|نظم|نظّم).{0,18}(?:فكرتي|افكاري|أفكاري|أفكار)/i.test(text)) return respond("idea", `${stylePrefix()}نرتبها من دون تحويلها لمهام: اكتب الفكرة كما هي، وأنا أفصل لك الهدف والمشكلة وأبسط بداية وما نؤجله.`);
    ...
    if (/(?:لخص|تلخيص|فرغ|تفريغ).{0,30}(?:اجتماع|محاضرة|مكالمة|تسجيل|الصوت)/i.test(text) || /(?:اجتماع|محاضرة).{0,20}(?:طويل|كامل)/i.test(text)) { respond("summary", `${stylePrefix()}نفتح لك «لخّص اجتماعك». للاجتماع الطويل اختر 15 دقيقة على الجوال أو 30 دقيقة كخيار متوازن؛ كل جزء يُحفظ محليًا ويستأنف التفريغ عند العودة، ثم يجمع NAVIXA النتيجة في ملخص واحد قابل للمراجعة والتصدير.`); window.setTimeout(() => { location.href = "/meetings"; }, 450); return; }
