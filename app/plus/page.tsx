...
export const metadata: Metadata = {
  ...
  description: "NAVIXA هِمّة لمدة شهر: أدوات الاستماع والتنبيه ومراقبة الشاشة والملخصات والاستمرارية بين الكمبيوتر والجوال، مع English Learning وKids وFitness.",
};
...
const included = [
  ...
  ["◫", "منظومة NAVIXA التابعة", "تشمل NAVIXA English Learning وNAVIXA Kids وNAVIXA Fitness ضمن هِمّة."],
...
export default function NavixaPlusPage() {
  return <main className="plus-page plus-simple" dir={languageIdentity.ar.direction}>
    ...
    <section className="plus-hero" aria-labelledby="plus-title"><div className="plus-hero-copy"><span className="plus-eyebrow">NAVIXA هِمّة · شهر كامل</span><h1 id="plus-title">خلك مستمر<br /><strong>بهمّة.</strong></h1><p>باقة للاستمرار بثبات في الدراسة والعمل والاجتماعات، تجمع أدوات الانتباه والمتابعة والملخصات لمدة شهر كامل، وتشمل مشاريع NAVIXA التابعة.</p><div className="plus-hero-actions"><Link href="/pricing" className="primary">شاهد السعر والمقارنة</Link><a href="#included" className="ghost">شاهد ما تشمله هِمّة</a><Link className="ghost" href="/sprint">أحتاج عَزْم لخمسة أيام</Link></div><div className="plus-trust"><span>✓ السعر الرسمي <PlanPriceInline plan="monthly" suffix=" / شهر" /></span><span>✓ English + Kids + Fitness</span><span>✓ كمبيوتر + جوال</span></div></div><aside className="plus-preview" aria-label="لمحة عن تجربة هِمّة"><span className="preview-badge">هِمّة</span><div className="preview-screen"><div className="preview-dot" /><small>يومك مستمر…</small><b>تم سماع اسمك</b><p>NAVIXA ينتبه معك حتى تواصل بهِمّة.</p><span className="preview-phone">إشعار على الجوال أيضًا</span></div></aside></section>
    ...
    <section className="plus-confidence" aria-labelledby="confidence-title"><div className="plus-confidence-copy"><small>هِمّة أو عَزْم؟</small><h2 id="confidence-title">اختر حسب المدة والطاقة التي تحتاجها.</h2><p><strong>هِمّة:</strong> شهر كامل بسعر <PlanPriceInline plan="monthly" /> مع منظومة NAVIXA التابعة. <strong>عَزْم:</strong> خمسة أيام بسعر <PlanPriceInline plan="sprint" /> لأدوات الجلسة الأساسية فقط.</p><Link href="/pricing">شاهد جدول المقارنة الكامل ←</Link></div><div className="plus-confidence-grid">{trust.map(([title,description])=><article key={title}><b>{title}</b><p>{description}</p></article>)}</div></section>
    <section className="plus-status" aria-labelledby="plus-status-title"><div><span className="plus-status-icon">i</span><div><small>السعر والشراء</small><h2 id="plus-status-title">سعر هِمّة معلن بوضوح</h2><p>السعر الأساسي الحالي <PlanPriceInline plan="monthly" suffix=" لمدة شهر واحد" />. وعند فتح الدفع سيُقرأ السعر من إعدادات الخادم نفسها، ثم يظهر المبلغ النهائي مرة أخرى قبل إتمام العملية.</p></div></div><Link href="/pricing">قائمة الأسعار والمقارنة ←</Link></section>
    <InterestForm />
