...
export default function NavixaPlusPage() {
  return <main className="plus-page plus-simple" dir={languageIdentity.ar.direction}>
    ...
    <section className="plus-hero" aria-labelledby="plus-title"><div className="plus-hero-copy"><span className="plus-eyebrow">NAVIXA Plus</span><h1 id="plus-title">خلّ NAVIXA<br /><strong>ينتبه معك.</strong></h1><p>Plus ليس مجرد مزايا إضافية. هو الطبقة التي تجعل NAVIXA يراقب الأشياء الصغيرة التي تستهلك انتباهك، ثم ينبهك عندما تحتاج فعلًا أن تنظر.</p><div className="plus-hero-actions"><a href="#interest" className="primary">أبلغني عند فتح Plus</a><a className="ghost" href="#included">شاهد ما الذي ستحصل عليه</a></div><div className="plus-trust"><span>✓ لا دفع حاليًا</span><span>✓ لا بطاقة</span><span>✓ خصوصية واضحة</span></div></div><aside className="plus-preview" aria-label="لمحة عن تجربة Plus"><span className="preview-badge">مثال</span><div className="preview-screen"><div className="preview-dot" /><small>اجتماعك مستمر…</small><b>تم سماع اسمك</b><p>NAVIXA انتبه لك. ارجع للاجتماع الآن.</p><span className="preview-phone">إشعار على الجوال أيضًا</span></div></aside></section>
    ...
    <section className="plus-status" aria-labelledby="plus-status-title"><div><span className="plus-status-icon">i</span><div><small>الحالة الحالية</small><h2 id="plus-status-title">Plus جاهز للتجهيز، والدفع ننتظر اعتماده</h2><p>نقدر الآن نسجل اهتمامك فقط. عند فتح بوابة الدفع الرسمية ستظهر الخطة والسعر ومدة الاشتراك وكل التفاصيل قبل أي تحصيل.</p></div></div><Link href="/account">إدارة حسابك ←</Link></section>
    <InterestForm />
    <section className="plus-promise"><div><small>وعد NAVIXA</small><h2>بياناتك ليست ثمن الاشتراك.</h2><p>لن تحفظ NAVIXA بيانات بطاقات أو محافظ. وعند إتاحة الدفع رسميًا سيكون عبر مزود معتمد وتحقيق خادمي آمن.</p><p><Link href="/terms">الشروط والأحكام</Link> · <Link href="/privacy">سياسة الخصوصية</Link> · <Link href="/refund-policy">الإلغاء والاسترداد</Link></p></div></section>
