...
export async function POST(request:Request){
  ...
  if(action==="create_founders"){try{await database.prepare("INSERT INTO navixa_discount_codes (id,code,discount_type,discount_value,plans,max_redemptions,redeemed_count,reserved_count,valid_from,valid_until,enabled,created_at,updated_at) VALUES (?, 'FOUNDERS100','percent',20,'all',100,0,0,?,'2026-09-19',1,?,?)").bind(crypto.randomUUID(),now,now,now).run();return NextResponse.json({ok:true,message:"تم إنشاء FOUNDERS100 بخصم 20% لأول 100 عميل حتى 19 سبتمبر 2026"});}catch{return NextResponse.json({error:"كود FOUNDERS100 موجود مسبقًا. يمكنك مراجعته من القائمة."},{status:409});}}
