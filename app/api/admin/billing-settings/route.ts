...
const settingKeys=["provider","mode","public_checkout","test_webhook_enabled","live_payments_enabled","card_mada_enabled","apple_pay_enabled","stc_pay_enabled","samsung_pay_enabled"] as const;
type SettingKey=(typeof settingKeys)[number];
const defaults:Record<SettingKey,string>={provider:"moyasar",mode:"test",public_checkout:"false",test_webhook_enabled:"false",live_payments_enabled:"false",card_mada_enabled:"true",apple_pay_enabled:"false",stc_pay_enabled:"false",samsung_pay_enabled:"false"};
...
export async function POST(request:Request){
  if(!await allowed(request))return NextResponse.json({error:"غير مصرح"},{status:401,headers:{"Cache-Control":"no-store"}});
  const body=await request.json().catch(()=>({})) as {provider?:unknown;mode?:unknown;publicCheckout?:unknown;testWebhookEnabled?:unknown;livePaymentsEnabled?:unknown;cardMadaEnabled?:unknown;applePayEnabled?:unknown;stcPayEnabled?:unknown;samsungPayEnabled?:unknown};
  ...
  if(next.card_mada_enabled!=="true"&&next.apple_pay_enabled!=="true"&&next.stc_pay_enabled!=="true"&&next.samsung_pay_enabled!=="true")return NextResponse.json({error:"فعّل وسيلة دفع واحدة على الأقل قبل حفظ الإعدادات"},{status:400});
