...
export async function GET(request:Request){
  ...
  if(!identity)return NextResponse.json({error:"غير مصرح"},{status:401,headers:{"Cache-Control":"no-store"}});
...
export async function POST(request:Request){
  ...
  const changed=settingKeys.filter(key=>current[key]!==next[key]);
  const now=new Date().toISOString();
  try{
    ...
    await writeAdminActivity(database,{adminEmail:identity.email,action:"billing_settings.update",resource:"billing",outcome:"failure",metadata:{changed}});
    return NextResponse.json({error:"تعذر حفظ إعدادات الدفع"},{status:500,headers:{"Cache-Control":"no-store"}});
  }
  await writeAdminActivity(database,{adminEmail:identity.email,action:"billing_settings.update",resource:"billing",metadata:{provider:next.provider,mode:next.mode,publicCheckout:next.public_checkout==="true",livePayments:next.live_payments_enabled==="true",changed}});
