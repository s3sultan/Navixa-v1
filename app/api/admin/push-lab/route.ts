...
export async function POST(request:Request){
  const identity=await requireAdminPermission(request,"push.test");
  if(!identity)return reply({error:"جلسة الإدارة غير صالحة أو لا تملك صلاحية اختبار Push"},401);
  ...
  if(!result.ok){
    if(result.status===404||result.status===410)await database.prepare("DELETE FROM navixa_push_subscriptions WHERE endpoint=?").bind(endpoint).run();
    await writeAdminActivity(database,{adminEmail:identity.email,action:"push_lab.send",resource:"push",outcome:"failure",metadata:{kind,status:result.status||0}});
  ...
  await writeAdminActivity(database,{adminEmail:identity.email,action:"push_lab.send",resource:"push",metadata:{kind,urgency,requireInteraction:body.requireInteraction===true,silent:body.silent===true}});
