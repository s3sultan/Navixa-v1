...
export async function POST(request:Request){
  ...
  if(action==="set_profile_status"){const id=clean(body.profileId,80),status=clean(body.profileStatus,12);if(!id||!["active","paused"].includes(status))return NextResponse.json({error:"بيانات الرمز غير صالحة"},{status:400});await database.prepare("UPDATE navixa_referral_profiles SET status=? WHERE id=?").bind(status,id).run();return NextResponse.json({ok:true,message:status==="active"?"تم تشغيل رمز الإحالة":"تم إيقاف رمز الإحالة"},{headers:{"Cache-Control":"no-store"}})}
  if(action==="delete_profile"){const id=clean(body.profileId,80);if(!id)return NextResponse.json({error:"معرف الرمز مطلوب"},{status:400});const used=await database.prepare("SELECT id FROM navixa_referral_attributions WHERE referral_profile_id=? LIMIT 1").bind(id).all();if(used.results.length)return NextResponse.json({error:"لا يمكن حذف رمز استُخدم؛ أوقفه بدلًا من ذلك"},{status:409});await database.prepare("DELETE FROM navixa_referral_profiles WHERE id=?").bind(id).run();return NextResponse.json({ok:true,message:"تم حذف رمز الإحالة غير المستخدم"},{headers:{"Cache-Control":"no-store"}})}
  if(action==="reverse_reward"){const id=clean(body.rewardId,80);if(!id)return NextResponse.json({error:"معرف المكافأة مطلوب"},{status:400});await database.prepare("UPDATE navixa_referral_rewards SET status='reversed',reversed_at=?,updated_at=? WHERE id=? AND status<>'applied'").bind(now,now,id).run();return NextResponse.json({ok:true,message:"تم عكس المكافأة غير المستخدمة"},{headers:{"Cache-Control":"no-store"}})}
