...
export async function POST(request:Request){
  if(!await allowed(request))return NextResponse.json({error:"غير مصرح"},{status:401,headers:{"Cache-Control":"no-store"}});
  const database=await db();if(!database)return NextResponse.json({error:"التخزين غير مهيأ"},{status:503,headers:{"Cache-Control":"no-store"}});await ensureReferralSchema(database);const body=await request.json().catch(()=>({})) as {action?:unknown;contact?:unknown;displayName?:unknown;enabled?:unknown;monthlyRewardDays?:unknown;quarterlyRewardDays?:unknown;maxRewardsPerMonth?:unknown;attributionDays?:unknown;rewardId?:unknown};const action=clean(body.action,24),now=new Date().toISOString();
