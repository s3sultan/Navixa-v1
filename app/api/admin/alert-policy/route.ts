...
import {ALERT_POLICY_TYPES,defaultAlertPolicy,isAlertPolicy,readAlertMessages,readAlertPolicy,writeAlertPolicy,type AlertMessageMap,type AlertPolicyDatabase,type AlertPolicyMap} from "../../../../worker/alertPolicy.ts";
...
export async function GET(request:Request){
  ...
  const database=await db();if(!database)return reply({policy:defaultAlertPolicy(),messages:{},configured:false},503);
  try{
    const [policy,messages]=await Promise.all([readAlertPolicy(database),readAlertMessages(database)]);
    return reply({policy,messages,configured:true});
  }catch{return reply({error:"تعذر قراءة سياسة التنبيهات"},500)}
}

export async function POST(request:Request){
  if(!await allowed(request))return reply({error:"غير مصرح"},401);
  const database=await db();if(!database)return reply({error:"التخزين غير مهيأ"},503);
  const body=await request.json().catch(()=>({})) as {policy?:unknown;messages?:unknown};
  ...
  const incomingMessages=body.messages&&typeof body.messages==="object"?body.messages as Record<string,unknown>:{};
  ...
  for(const type of ALERT_POLICY_TYPES){
    ...
    const message=incomingMessages[type];if(typeof message==="string"&&message.trim())messages[type]=message.trim().slice(0,240);
  }
  try{await writeAlertPolicy(database,policy as AlertPolicyMap,messages);return reply({ok:true,policy,messages})}catch{return reply({error:"تعذر حفظ سياسة التنبيهات"},500)}
}
