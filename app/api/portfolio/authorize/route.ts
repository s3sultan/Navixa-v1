...
import { createPortfolioGrant, portfolioApp, PORTFOLIO_APPS, resolvePortfolioMembership, type PortfolioApp } from "../../../../worker/portfolioAccess.ts";
...
type WorkerBinding = { env?: { DB?: Database; NAVIXA_PORTFOLIO_TOKEN_SECRET?: string } };
...
async function bindings() {
  ...
  catch { return globalThis as { DB?: Database; NAVIXA_PORTFOLIO_TOKEN_SECRET?: string }; }
}
...
export async function GET(request: Request) {
  ...
  if (!env.DB || !env.NAVIXA_PORTFOLIO_TOKEN_SECRET) return NextResponse.json({ error: "منظومة NAVIXA غير جاهزة مؤقتًا" }, { status: 503, headers });
  ...
  const grant = await createPortfolioGrant({ app: requested, membership, secret: env.NAVIXA_PORTFOLIO_TOKEN_SECRET });
