...
type Settings={provider:string;mode:string;public_checkout:string;test_webhook_enabled:string;live_payments_enabled:string;card_mada_enabled:string;apple_pay_enabled:string;stc_pay_enabled:string;samsung_pay_enabled:string};
...
const defaults:Settings={provider:"moyasar",mode:"test",public_checkout:"false",test_webhook_enabled:"false",live_payments_enabled:"false",card_mada_enabled:"true",apple_pay_enabled:"false",stc_pay_enabled:"false",samsung_pay_enabled:"false"};
...
export async function POST(request:Request){
  ...
  const methods:string[]=[];if(current.card_mada_enabled!=="false")methods.push("creditcard");if(current.apple_pay_enabled==="true")methods.push("applepay");if(current.stc_pay_enabled==="true")methods.push("stcpay");if(current.samsung_pay_enabled==="true")methods.push("samsungpay");if(!methods.length)return NextResponse.json({error:"لا توجد وسيلة دفع مفعلة حاليًا"},{status:503,headers:{"Cache-Control":"no-store"}});
  const origin=new URL(request.url).origin;
  return NextResponse.json({intentId,provider:"moyasar",publicKey,amount,currency:"SAR",description:foundersIntentId?"NAVIXA Plus · سعر مؤسس":""+details.label,callbackUrl:`${origin}/plus/complete?intent=${encodeURIComponent(intentId)}`,metadata:{navixa_intent:intentId,navixa_plan:plan,navixa_founders_intent:foundersIntentId},methods,supportedNetworks:["mada","visa","mastercard","amex","unionpay"],applePay:current.apple_pay_enabled==="true"?{country:"SA",label:"NAVIXA Plus",validateMerchantUrl:"https://api.moyasar.com/v1/applepay/initiate"}:null,referralApplied:Boolean(attribution)},{headers:{"Cache-Control":"no-store"}});
}
