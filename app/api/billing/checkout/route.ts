...
import { buildMoyasarCheckout, type PaymentMethodId } from "../../../billing/providers/index.ts";
...
type Settings={provider:string;mode:string;public_checkout:string;test_webhook_enabled:string;live_payments_enabled:string;card_mada_enabled:string;visa_enabled:string;mastercard_enabled:string;apple_pay_enabled:string;stc_pay_enabled:string;samsung_pay_enabled:string;tamara_enabled:string;tamara_sandbox_enabled:string};
type Plan="monthly"|"quarterly";
type FoundersAward={intent_id:string;contact:string;user_id:string;price_amount:number;status:string;expires_at:string};

const defaults:Settings={provider:"moyasar",mode:"test",public_checkout:"false",test_webhook_enabled:"false",live_payments_enabled:"false",card_mada_enabled:"true",visa_enabled:"true",mastercard_enabled:"true",apple_pay_enabled:"false",stc_pay_enabled:"false",samsung_pay_enabled:"false",tamara_enabled:"false",tamara_sandbox_enabled:"true"};
...
export async function POST(request:Request){
  ...
  const contact=session.email,plan=clean(body.plan,20) as Plan,referralCode=cleanReferral(body.referralCode,20).toUpperCase(),discountCode=clean(body.discountCode,32).toUpperCase(),foundersIntentId=clean(body.foundersIntentId,80);
  if(!validEmail(contact)||!(plan in planDetails))return NextResponse.json({error:"بيانات الاشتراك غير صالحة"},{status:400,headers:{"Cache-Control":"no-store"}});
  if(foundersIntentId&&plan!=="monthly")return NextResponse.json({error:"سعر المؤسس يطبق على الباقة الشهرية فقط"},{status:400,headers:{"Cache-Control":"no-store"}});
  if(foundersIntentId&&discountCode)return NextResponse.json({error:"لا يمكن دمج سعر المؤسس مع كود خصم آخر"},{status:400,headers:{"Cache-Control":"no-store"}});
  const now=new Date(),details=planDetails[plan],intentId=crypto.randomUUID();let amount=details.amount,discountApplied="";
  if(foundersIntentId){
    const awards=await database.prepare("SELECT intent_id,contact,user_id,price_amount,status,expires_at FROM navixa_founders_awards WHERE intent_id=? AND contact=? AND user_id=? AND status='reserved' AND expires_at>? LIMIT 1").bind(foundersIntentId,contact,session.userId,now.toISOString()).all<FoundersAward>();
    const award=awards.results[0];if(!award)return NextResponse.json({error:"سعر المؤسس غير متاح أو انتهت مهلة الحجز"},{status:409,headers:{"Cache-Control":"no-store"}});amount=award.price_amount;
  }else if(discountCode){
    await database.prepare("UPDATE navixa_discount_codes SET reserved_count=(SELECT COUNT(*) FROM navixa_billing_intents WHERE discount_code=? AND status='pending' AND expires_at>?),updated_at=? WHERE code=?").bind(discountCode,now.toISOString(),now.toISOString(),discountCode).run();
    const rows=await database.prepare("SELECT code,discount_type,discount_value,plans,max_redemptions,redeemed_count,reserved_count,valid_from,valid_until,enabled FROM navixa_discount_codes WHERE code=? LIMIT 1").bind(discountCode).all<{code:string;discount_type:string;discount_value:number;plans:string;max_redemptions:number;redeemed_count:number;reserved_count:number;valid_from:string;valid_until:string;enabled:number}>();
    const coupon=rows.results[0],nowMs=Date.now(),from=coupon?.valid_from?new Date(coupon.valid_from).getTime():0,until=coupon?.valid_until?new Date(coupon.valid_until+"T23:59:59Z").getTime():0,alreadyUsed=await database.prepare("SELECT id FROM navixa_billing_intents WHERE contact=? AND discount_code=? AND status='paid' LIMIT 1").bind(contact,discountCode).all<{id:string}>(),eligible=Boolean(coupon&&coupon.enabled&&(!from||nowMs>=from)&&(!until||nowMs<=until)&&(!coupon.max_redemptions||coupon.redeemed_count+coupon.reserved_count<coupon.max_redemptions)&&(coupon.plans==="all"||coupon.plans===plan)&&!alreadyUsed.results.length);
    if(!eligible)return NextResponse.json({error:"كود الخصم غير صالح أو انتهى أو تم استخدامه لهذا الحساب"},{status:400,headers:{"Cache-Control":"no-store"}});
    const reduction=coupon.discount_type==="fixed"?coupon.discount_value:Math.round(details.amount*coupon.discount_value/100);amount=Math.max(100,details.amount-reduction);discountApplied=coupon.code;
    const reserved=await database.prepare("UPDATE navixa_discount_codes SET reserved_count=reserved_count+1,updated_at=? WHERE code=? AND enabled=1 AND (max_redemptions=0 OR redeemed_count+reserved_count<max_redemptions)").bind(now.toISOString(),discountCode).run();if(changed(reserved)===0)return NextResponse.json({error:"اكتمل عدد استخدامات كود الخصم، حاول دون الكود."},{status:409,headers:{"Cache-Control":"no-store"}});
  }
  const expires=new Date(now.getTime()+15*60_000).toISOString();
  await database.prepare("INSERT INTO navixa_billing_intents (id,user_id,contact,plan,amount,currency,mode,status,provider_payment_id,referral_code,discount_code,founders_intent_id,created_at,updated_at,expires_at) VALUES (?,?,?,?,?,'SAR','live','pending','',?,?,?,?,?,?)").bind(intentId,session.userId,contact,plan,amount,referralCode,discountApplied,foundersIntentId,now.toISOString(),now.toISOString(),expires).run();
  const attribution=referralCode?await createAttribution(database,referralCode,contact,intentId).catch(()=>null):null;
  const enabledMethods:PaymentMethodId[]=[];if(current.card_mada_enabled!=="false")enabledMethods.push("mada");if(current.visa_enabled!=="false")enabledMethods.push("visa");if(current.mastercard_enabled!=="false")enabledMethods.push("mastercard");if(current.apple_pay_enabled==="true")enabledMethods.push("applepay");if(current.stc_pay_enabled==="true")enabledMethods.push("stcpay");if(current.samsung_pay_enabled==="true")enabledMethods.push("samsungpay");
  const hasCardNetwork=enabledMethods.some(method=>method==="mada"||method==="visa"||method==="mastercard");
  const supportedNetworks=[...(current.card_mada_enabled!=="false"?["mada"]:[]),...(current.visa_enabled!=="false"?["visa"]:[]),...(current.mastercard_enabled!=="false"?["mastercard"]:[]),...(hasCardNetwork?["amex","unionpay"]:[])];
  const origin=new URL(request.url).origin;
  const checkout=buildMoyasarCheckout({id:intentId,amount,currency:"SAR",description:foundersIntentId?"NAVIXA Plus · سعر مؤسس":details.label,callbackUrl:`${origin}/plus/complete?intent=${encodeURIComponent(intentId)}`,metadata:{navixa_intent:intentId,navixa_plan:plan,navixa_founders_intent:foundersIntentId}},publicKey,enabledMethods);
  if(checkout.state!=="ready")return NextResponse.json({error:"لا توجد وسيلة دفع مفعلة حاليًا"},{status:503,headers:{"Cache-Control":"no-store"}});
  return NextResponse.json({intentId,provider:checkout.provider,publicKey:checkout.publicKey,amount:checkout.amount,currency:checkout.currency,description:foundersIntentId?"NAVIXA Plus · سعر مؤسس":details.label,callbackUrl:checkout.callbackUrl,metadata:checkout.metadata,methods:checkout.methods,supportedNetworks,applePay:checkout.applePay,referralApplied:Boolean(attribution)},{headers:{"Cache-Control":"no-store"}});
}
