...
async function schema(database: D1Database) {
  ...
  for (const [key, value] of Object.entries(defaults)) await database.prepare("INSERT OR IGNORE INTO navixa_billing_settings (setting_key,setting_value,updated_at) VALUES (?,?,?)").bind(key, value, now).run();
}
async function settings(database: D1Database) {
  await schema(database);
  const rows = await database.prepare("SELECT setting_key,setting_value FROM navixa_billing_settings").all<{ setting_key: string; setting_value: string }>();
  const next = { ...defaults };
  for (const row of rows.results) if (row.setting_key in next) next[row.setting_key as keyof Settings] = row.setting_value;
  return next;
}
...
export async function POST(request: Request) {
  ...
  if (!database) return NextResponse.json({ error: "بوابة الدفع غير مهيأة" }, { status: 503 });
  ...
  if (!expectedSecret || !enabled) return NextResponse.json({ error: "بوابة الدفع مقفلة من الإدارة" }, { status: 503 });

  const normalized = normalizeMoyasarWebhook({ body: { ...body, secret_token: webhookSecret(request, body) }, expectedSecret });
  if (!normalized) return NextResponse.json({ error: "توقيع Webhook غير صالح" }, { status: 401 });
  if (requestedLive && (current.mode !== "live" || current.public_checkout !== "true")) return NextResponse.json({ error: "الدفع الحي غير مفعل للزوار" }, { status: 409 });

  const { eventId, eventType, intentId, paymentId: providerPaymentId } = normalized;
  const intents = await database.prepare("SELECT id,user_id,contact,plan,amount,currency,status,founders_intent_id FROM navixa_billing_intents WHERE id=? AND mode='live' AND status='pending' AND expires_at>? LIMIT 1")
    .bind(intentId, new Date().toISOString())
    .all<{ id: string; user_id: string; contact: string; plan: string; amount: number; currency: string; status: string; founders_intent_id: string }>();
  const intent = intents.results[0];
  const contact = intent?.contact || "";
  const plan = intent?.plan || "";
  const userId = intent?.user_id || "";
  if (!eventId || !intentId || !userId || !validEmail(contact) || !normalized.paid || !["monthly", "sprint"].includes(plan)) return NextResponse.json({ error: "حدث الدفع لا يحمل نية NAVIXA صالحة" }, { status: 400 });

  if (requestedLive) {
    if (!secrets.MOYASAR_LIVE_SECRET_KEY) return NextResponse.json({ error: "مفتاح تحقق مُيسر الحي غير مهيأ" }, { status: 503 });
    if (normalized.amount !== intent.amount || normalized.currency !== intent.currency) return NextResponse.json({ error: "قيمة أو عملة الدفع لا تطابق نية NAVIXA" }, { status: 400 });
    const verified = await verifyMoyasarPayment({
      secret: secrets.MOYASAR_LIVE_SECRET_KEY,
      paymentId: providerPaymentId,
      expectedIntentId: intentId,
      expectedAmount: intent.amount,
      expectedCurrency: intent.currency,
    }).catch(() => null);
    if (!verified || verified.status !== "paid") return NextResponse.json({ error: "تعذر التحقق من الدفعة لدى مُيسر" }, { status: 409 });
  }

  const existing = await database.prepare("SELECT id FROM navixa_billing_events WHERE provider_event_id=? LIMIT 1").bind(eventId).all<{ id: string }>();
  if (existing.results.length) return NextResponse.json({ ok: true, duplicate: true });

  const now = new Date().toISOString();
  const days = plan === "sprint" ? 5 : 30;
  const end = new Date(Date.now() + days * 86400000).toISOString();
  const subscriberId = crypto.randomUUID();
  const mode = requestedLive ? "live" : "test";
  let foundersAwarded = false;

  if (requestedLive) {
    await database.prepare("INSERT INTO navixa_subscribers (id,user_id,contact,display_name,plan,status,subscription_ends_at,source,created_at,updated_at) VALUES (?,?,?,'',?,'active',?,'moyasar_webhook',?,?) ON CONFLICT(contact) DO UPDATE SET user_id=excluded.user_id,plan=excluded.plan,status='active',subscription_ends_at=excluded.subscription_ends_at,source='moyasar_webhook',updated_at=excluded.updated_at")
      .bind(subscriberId, userId, contact, plan, end, now, now).run();
    await database.prepare("UPDATE navixa_billing_intents SET status='paid',provider_payment_id=?,updated_at=? WHERE id=? AND status='pending'").bind(providerPaymentId, now, intentId).run();
    foundersAwarded = plan === "monthly" && intent.founders_intent_id ? await completeFoundersAward(database, intent.founders_intent_id).catch(() => false) : false;
  }
