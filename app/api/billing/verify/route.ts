...
type Intent={id:string;contact:string;plan:"monthly"|"quarterly";amount:number;currency:string;status:string;expires_at:string};
...
export async function POST(request:Request){
  ...
  const selected=await database.prepare("SELECT id,contact,plan,amount,currency,status,expires_at FROM navixa_billing_intents WHERE id=? LIMIT 1").bind(intentId).all<Intent>();const intent=selected.results[0];if(!intent||intent.status!=="pending"||new Date(intent.expires_at).getTime()<Date.now())return NextResponse.json({error:"نية الشراء غير متاحة أو انتهت"},{status:400,headers:{"Cache-Control":"no-store"}});
