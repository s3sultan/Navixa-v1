...
type SubscriptionPayload={endpoint?:unknown;keys?:{p256dh?:unknown;auth?:unknown};competitions?:unknown;teams?:unknown;beforeMinutes?:unknown};
...
async function schema(database:D1Database){await database.prepare("CREATE TABLE IF NOT EXISTS navixa_push_subscriptions (id TEXT PRIMARY KEY, endpoint TEXT NOT NULL UNIQUE, p256dh TEXT NOT NULL, auth TEXT NOT NULL, competitions_json TEXT NOT NULL DEFAULT '[]', teams_json TEXT NOT NULL DEFAULT '[]', before_minutes INTEGER NOT NULL DEFAULT 10, enabled INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)").run()}
