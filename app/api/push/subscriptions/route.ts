...
async function schema(database:D1Database){
  await database.prepare("CREATE TABLE IF NOT EXISTS navixa_push_subscriptions (id TEXT PRIMARY KEY, endpoint TEXT NOT NULL UNIQUE, p256dh TEXT NOT NULL, auth TEXT NOT NULL, competitions_json TEXT NOT NULL DEFAULT '[]', teams_json TEXT NOT NULL DEFAULT '[]', before_minutes INTEGER NOT NULL DEFAULT 10, before_minutes_json TEXT NOT NULL DEFAULT '[]', user_id TEXT NOT NULL DEFAULT '', enabled INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)").run();
  try{await database.prepare("ALTER TABLE navixa_push_subscriptions ADD COLUMN before_minutes_json TEXT NOT NULL DEFAULT '[]'").run()}catch{/* Column already exists on established databases. */}
  try{await database.prepare("ALTER TABLE navixa_push_subscriptions ADD COLUMN user_id TEXT NOT NULL DEFAULT ''").run()}catch{/* Column already exists on established databases. */}
}
...
export async function POST(request:Request){
  ...
  await database.prepare("INSERT INTO navixa_push_subscriptions (id,endpoint,p256dh,auth,competitions_json,teams_json,before_minutes,before_minutes_json,user_id,enabled,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(endpoint) DO UPDATE SET p256dh=excluded.p256dh,auth=excluded.auth,competitions_json=excluded.competitions_json,teams_json=excluded.teams_json,before_minutes=excluded.before_minutes,before_minutes_json=excluded.before_minutes_json,user_id=CASE WHEN excluded.user_id<>'' THEN excluded.user_id ELSE navixa_push_subscriptions.user_id END,enabled=1,updated_at=excluded.updated_at").bind(crypto.randomUUID(),endpoint,p256dh,auth,JSON.stringify(competitions),JSON.stringify(teams),beforeMinutes[0],JSON.stringify(beforeMinutes),userId,1,now,now).run();
  return NextResponse.json({ok:true,accountBound:Boolean(userId)},{headers:{"Cache-Control":"no-store"}});
}
