...
export async function POST(request:Request){
  try{
    ...
    if(!googleClientId)return NextResponse.json({error:"لم تكتمل تهيئة تسجيل الدخول الآمن"},{status:503});
    ...
    if(profile.aud!==googleClientId||!verified)return NextResponse.json({error:"حساب Google غير موثّق"},{status:401});
