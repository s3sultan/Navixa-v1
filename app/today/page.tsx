  ...
  const submitQuickAdd = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (kind === "appointment") {
      ...
      if(Number.isFinite(dueAt.getTime())&&dueAt.getTime()>Date.now()){
        void fetch("/api/reminders",{method:"POST",headers:{"content-type":"application/json"},credentials:"same-origin",body:JSON.stringify({title:`موعد: ${cleanTitle}`,dueAt:dueAt.toISOString(),email:false,telegram:false,push:true,source:"schedule"})}).catch(()=>null);
      }
      setNotice("حُفظ الموعد، وPush الجدول يعمل إذا فعّلته من مركز التنبيهات");
    }
    if (kind === "focus") {
  ...
  return <main className="today-page" dir="rtl">
    <FeatureAccessGate feature="صفحة يومي والإضافة السريعة">
      ...
      <header className="today-topbar">
        ...
        <nav aria-label="تنقل صفحة يومي"><Link href="/">الرئيسية</Link><Link href="/progress">تقدّمي</Link><Link href="/reminders">التذكيرات</Link><Link href="/portfolio">المنظومة</Link><Link href="/account">حسابي</Link></nav>
      </header>
