...
export default function EyeRestClock(){
  ...
  const skip=()=>{setResting(false);setElapsed(0);setRestSeconds(REST);setRestStep(0)}, minutesLeft=Math.ceil((CYCLE-elapsed)/60);
  return <article className="eye-rest-card">
    ...
    <figure className={`eye-guide-image ${resting?"resting":""}`}><img src="/health-guides/eye-rest.webp" alt="رجل ينظر بهدوء إلى نقطة بعيدة لإراحة العين"/><span className="eye-guide-glow"/><figcaption><i/>{resting?"أبعد نظرك عن الشاشة":"انظر إلى نقطة بعيدة"}</figcaption>{resting&&<b className="eye-rest-count">{restSeconds}</b>}</figure>
