...
export default function PrayerStrip(){
  ...
  const prayerTimeline=(()=>{if(!timings)return null;const events=prayerOrder.map(name=>({name,time:parseTime(timings[name])}));let nextIndex=events.findIndex(event=>now<event.time),nextTime:Date,startTime:Date,nextName:PrayerName;if(nextIndex===-1){nextIndex=0;nextName=events[0].name;nextTime=addMinutes(events[0].time,1440);startTime=events[events.length-1].time}else{nextName=events[nextIndex].name;nextTime=events[nextIndex].time;startTime=nextIndex===0?addMinutes(events[events.length-1].time,-1440):events[nextIndex-1].time}const totalMs=Math.max(1,nextTime.getTime()-startTime.getTime()),elapsedMs=Math.min(totalMs,Math.max(0,now.getTime()-startTime.getTime()));return {name:nextName,progress:Math.round(elapsedMs/totalMs*100)}})();
  ...
  return <div className={`prayer-strip ${expanded?"is-expanded":""}`}>
    <button type="button" className="prayer-mobile-current" onClick={()=>setExpanded(v=>!v)} aria-expanded={expanded}><div><small>{nextTarget?.phase==="iqama"?"الإقامة القادمة":"الأذان القادم"}</small><b>{nextTarget?prayerLabels[nextTarget.name]:"الصلاة"}</b></div>{prayerTimeline&&<div className="prayer-mobile-timeline" aria-label={`التقدم نحو أذان ${prayerLabels[prayerTimeline.name]}`}><span className="prayer-mobile-timeline-labels"><small>الآن</small><small>{prayerLabels[prayerTimeline.name]}</small></span><span className="prayer-mobile-track"><i style={{width:`${prayerTimeline.progress}%`}}/><b style={{left:`${prayerTimeline.progress}%`}}/></span></div>}<div><small>المتبقي</small><strong>{nextTarget?formatRemain(nextTarget.remainMs):"—"}</strong></div></button>
