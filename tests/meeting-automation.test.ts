...
test("automatic transcription completion uses automation core and shared privacy-safe persistent history", async () => {
  const [studio, adapter, persistentClient] = await Promise.all([
    readFile(new URL("app/meetings/MeetingStudio.tsx", root), "utf8"),
    readFile(new URL("app/meetings/meetingAutomation.ts", root), "utf8"),
    readFile(new URL("app/automationRunHistoryClient.ts", root), "utf8"),
  ]);
  ...
  assert.match(adapter, /persistAutomationRunBestEffort\(run\)/);
  assert.match(adapter, /listPersistentAutomationRuns\(\{ skillId: MEETING_SUMMARY_SKILL_ID, limit: 50 \}\)/);
  assert.match(adapter, /Keep transcript and generated summary out of generic run history/);
  assert.match(persistentClient, /PERSISTENT_RUN_HISTORY_URL = "\/api\/automation-runs"/);
  assert.match(persistentClient, /keepalive: true/);
  assert.match(persistentClient, /credentials: "same-origin"/);
  assert.match(persistentClient, /Persistent history is observational only; it must never break local automation/);

  const payloadStart = persistentClient.indexOf("export function persistentRunPayload");
  const payloadEnd = persistentClient.indexOf("export function persistAutomationRunBestEffort");
  const payload = persistentClient.slice(payloadStart, payloadEnd);
