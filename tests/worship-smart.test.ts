...
assert.match(locationModel,/sessionPrayerLocation/);
assert.match(locationModel,/Precise device coordinates are deliberately session-memory only/);
assert.match(locationModel,/if\(location\.mode==="city"\)persistCityPrayerLocation\(location\)/);
assert.match(locationModel,/localStorage\.removeItem\(LEGACY_PRAYER_LOCATION_STORAGE_KEY\)/);
assert.doesNotMatch(locationModel,/JSON\.stringify\(location\)/);
assert.doesNotMatch(locationModel,/JSON\.stringify\(\{lat:/);
