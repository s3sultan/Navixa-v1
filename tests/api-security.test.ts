...
test("performance telemetry rejects cross-origin input and stores only bounded anonymous timings", async () => {
  ...
  try {
    ...
    const accepted = await reportPerformance(post("/api/performance", { path: "/health", ttfbMs: 123.4, loadMs: 456.7, lcpMs: 1750.5, inpMs: 86.2, clsMilli: 24.8, visitorId: "ignored" }));
    assert.equal(accepted.status, 200);
    const insert = statements.find(item => item.sql.startsWith("INSERT INTO navixa_performance_samples"));
    assert.ok(insert);
    assert.deepEqual(insert.values.slice(0,6), ["/health",123,457,1751,86,25]);
