...
const retry=60*60_000, maxAttempts=3, scheduleFreshness=10*60_000; const changes=(v:unknown)=>((v as {meta?:{changes?:number}})?.meta?.changes||0);
...
export async function ensureImportantReminderSchema(db:Db){
 ...
 await db.prepare("CREATE INDEX IF NOT EXISTS idx_navixa_important_reminders_source_due ON navixa_important_reminders(source,due_at)").run();
}
...
async function skipStaleSchedulePush(db:Db,row:Row,now:string){await db.prepare("UPDATE navixa_important_reminders SET push_status='skipped',updated_at=? WHERE id=? AND push_status IN ('pending','failed')").bind(now,row.id).run()}
