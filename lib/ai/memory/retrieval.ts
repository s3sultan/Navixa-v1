...
export function retrieveMemories(memories: NavixaMemory[], query: MemoryQuery): NavixaMemory[] {
...
function scoreMemory(memory: NavixaMemory, terms: string[]): number {
  ...
  return lexical * 0.45 + memory.salience * 0.25 + memory.confidence * 0.2 + sourceWeight * 0.1;
}

function tokenize(value: string): string[] {
  return Array.from(new Set(value.toLocaleLowerCase().split(/[^\p{L}\p{N}]+/u).map((part) => part.trim()).filter((part) => part.length >= 2)));
}
