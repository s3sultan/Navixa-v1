...
import type { MemoryQuery, MemoryStore, MemoryWriteInput, NavixaMemory, NavixaProjectScope } from "./types.ts";
...
export class NavixaDbMemoryStore implements MemoryStore {
  ...
  async list(query: MemoryQuery): Promise<NavixaMemory[]> {
    ...
    const sql = includeCore && query.project !== "core"
      ? `SELECT id,user_id,project,kind,content,source,sensitivity,confidence,salience,metadata_json,created_at,updated_at,expires_at
         FROM navixa_ai_memory WHERE user_id=? AND (project=? OR project='core')
         ORDER BY updated_at DESC LIMIT 100`
      : `SELECT id,user_id,project,kind,content,source,sensitivity,confidence,salience,metadata_json,created_at,updated_at,expires_at
         FROM navixa_ai_memory WHERE user_id=? AND project=?
         ORDER BY updated_at DESC LIMIT 100`;
  ...
  async upsert(input: MemoryWriteInput): Promise<NavixaMemory> {
    if (shouldRejectMemoryWrite(input)) throw new Error("memory-write-rejected");
    ...
    if (existing) {
      await execute(this.db, `UPDATE navixa_ai_memory
        ...
        value.source, value.sensitivity ?? "standard", value.confidence ?? 0.7,
        value.salience ?? 0.5, metadataJson, now, value.expiresAt ?? null, id, value.userId,
      ]);
    } else {
      await execute(this.db, `INSERT INTO navixa_ai_memory
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`, [
        id, value.userId, value.project, value.kind, value.content, value.source,
        value.sensitivity ?? "standard", value.confidence ?? 0.7, value.salience ?? 0.5,
        metadataJson, createdAt, now, value.expiresAt ?? null,
      ]);
    }
...
async function execute(db: MemorySqlDb, sql: string, values: unknown[]): Promise<void> {
  ...
  if (bound.run) await bound.run();
  else await bound.all();
}
