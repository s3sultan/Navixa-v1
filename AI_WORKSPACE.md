...
- الحالة: PR #193 أُغلق في 2026-09-14 دون دمج؛ لذلك حجز `app/ui-system/*`, `app/ui-lab/*`, `tests/navixa-ui-system.test.ts`, `tests/ui-smoke.mjs`, `tests/ui-system-smoke.mjs`, `package.json`, `PROJECT_MEMORY.md`, `AI_CHANGELOG.md`, `AI_WORKSPACE.md` لم يعد نشطًا.
- القيود: لا تُنقل أي تغييرات من الفرع المغلق إلى الإنتاج إلا بطلب ومراجعة مستقلة جديدة.
...
- القيود: لا تعديل لـ`package.json`، وتكون اختبارات الموقع الخارجية غير مدمرة ومحدودة النطاق.
...
- القيود: لم يُضف Runtime أو اعتماد جديد، ولا يوجد ادعاء تطبيق watchOS أصلي؛ Apple Watch تعتمد في هذه المرحلة على إشعارات Web Push المنعكسة من iPhone. أزرار الإجراءات تحسين تدريجي، ولم يُضف زر غفوة صوري قبل وجود جدولة خلفية حقيقية.

## مهمة موازية غير متقاطعة — NAVIXA Academic Structure + Memory Foundation

- الوكيل: ChatGPT.
- المهمة: بناء الأساس الذي يربط المقرر بطرحه ومكوّناته، ثم يضيف ذاكرة أكاديمية سياقية تفصل الحقيقة الحالية عن التاريخ والتوقع حسب الجامعة/المنطقة/الفرع/البرنامج/الخطة/السمستر.
- الحالة: التنفيذ والتحقق مكتملان على Draft PR #212 في الفرع `feat/academic-section-linkage-20260913`، والإنتاج غير متأثر. نجحت `Verify NAVIXA Pull Request` #601 و`NAVIXA Pre-Launch Gate` #508 و`NAVIXA Security Code Scan` #55 على final head `440a05ec4da619df6bca7f08fb2d086a02c8026e`، وهو نفس tree للـhead المنتج السابق مع commit صفري التغيير فقط.
- الملفات المحجوزة لهذه المهمة: محررة؛ لا يوجد حجز نشط حتى يطلب المستخدم المرحلة التالية أو الدمج.
- القيود: لا افتراض أن نمط الدراسة ثابت للمقرر أو المنطقة؛ نمط الحضور/عن بعد على مستوى الاجتماع نفسه. الاسم أو الرمز وحدهما لا يدمجان السجلات، والاستنتاج التاريخي يبقى Prediction ولا يصبح Fact تلقائيًا مهما تكرر. لم تُضف migration أو اعتماديات جديدة أو قيم أكاديمية وهمية.

## مهمة موازية غير متقاطعة — NAVIXA Multi-University Source Ingestion Contract

- الوكيل: ChatGPT.
- المهمة: بناء عقد إدخال بيانات أكاديمية يدعم جامعات كثيرة من البداية، بحيث تكون المؤسسة أول جدار فصل، وتدخل البيانات الرسمية وPDF/الصورة/الملفات/الإدخال اليدوي عبر بوابة تحقق قبل الذاكرة أو التخزين الدائم.
- الحالة: التنفيذ مكتمل على Draft PR #213 في الفرع `feat/multi-university-ingestion-20260913` كـstack فوق PR #212، والإنتاج غير متأثر. تم إنشاء PR تحقق مؤقت #214 إلى `master` فقط لتشغيل workflows، ونجحت `Verify NAVIXA Pull Request` #612 و`NAVIXA Pre-Launch Gate` #521 و`NAVIXA Security Code Scan` #68 على head `96c7d168365c70f43c175d8e0e3ec4922479bfeb` ثم أُغلق #214 دون دمج.
- الملفات المحجوزة لهذه المهمة: محررة؛ لا يوجد حجز نشط حتى يطلب المستخدم المرحلة التالية أو الدمج.
- القيود: `institutionId` إلزامي ولا يجوز استنتاج الجامعة من اسم المقرر أو رمزه؛ المصدر الرسمي لا يُقبل كـofficial إلا إذا طابق source مسجلًا للجامعة ونطاق HTTPS مسموحًا وتابعًا لنطاق رسمي للمؤسسة؛ المنطقة/الفرع/الخطة غير المعروفة تبقى غير محلولة ولا تُخمن. OCR والإدخال اليدوي غير الموثوق يذهبان للمراجعة، ولا D1 ولا migration ولا اعتماديات ولا UI في هذه المرحلة.

## آخر ما اكتمل

- اكتمل Multi-University Source Ingestion Contract داخل PR #213: سجل جامعات ومصادر صريح، فصل صارم بين المؤسسات، منع cross-university source، تحقق من official domains وlookalike hosts، قنوات API/portal/PDF/image/CSV/XLSX/manual، طرق structured/OCR/manual، وقرار `accept / review / reject` قبل تحويل البيانات إلى Observation. الاختبارات أثبتت أن نفس المقرر في جامعتين يبقى منفصلًا وأن البيانات المبهمة أو غير الموثوقة لا تتحول إلى حقيقة تلقائيًا.
- اكتمل Academic Memory Foundation داخل PR #212: هوية ثابتة للمقرر عبر تغيّر الرمز، Partition صارم حسب المؤسسة والمنطقة والفرع والكلية والبرنامج والخطة والمسار والمستوى، سجل append-only للمشاهدات، مصدر وزمن تحقق ودرجة ثقة، اختيار محافظ للحقيقة الحالية، ملف تاريخي حسب مكوّن المقرر، وتوقع صريح منفصل لا يتنكر كحقيقة.
- اكتمل أساس Academic Section Linkage داخل PR #212: نموذج Course Offering ومكوّنات محاضرة/معمل/تمارين، خيارات تسجيل كاملة، Evidence للرابط، رفض الخلط بين مكونات غير مترابطة، نمط دراسة مستقل لكل meeting، وUID/تنبيه فريد لكل meeting.
...
- في مهمة الأداء الحالية: إزالة تحميل Alexandria المكرر، تثبيت تخطيط قسم «يومك الآن» عند تبدل الخط، ثم قياس Lighthouse قبل اعتماد أي تحسين إضافي.
- بعد اعتماد PR #212 ثم #213: تصميم التخزين الدائم متعدد الجامعات في D1 مع Institution Registry فعلي، snapshots/versioning، deduplication، provenance، وسجل تغييرات append-only؛ لا يبدأ قبل إعادة توجيه PR #213 إلى `master` وإعادة الفحوص.
- انتظار مراجعة المستخدم لـDraft PR #212 وDraft PR #213 قبل أي دمج أو نشر.
