...
- الحالة: التنفيذ والتحقق مكتملان على Draft PR #212 في الفرع `feat/academic-section-linkage-20260913`، والإنتاج غير متأثر. نجحت `Verify NAVIXA Pull Request` #597 و`NAVIXA Pre-Launch Gate` #501 و`NAVIXA Security Code Scan` #48 على head `6f18c7d5005c4846f4c7dcdcb5339860718a3ec2`.
- الملفات المحجوزة لهذه المهمة: محررة؛ لا يوجد حجز نشط لهذه المهمة حتى يطلب المستخدم المرحلة التالية أو الدمج.
- القيود: لا افتراض أن نمط الدراسة ثابت للمقرر أو المنطقة؛ نمط الحضور/عن بعد على مستوى الاجتماع نفسه. التركيبات الرسمية/المراجعة فقط موثوقة للتوصية التلقائية، ولا تتحول العلاقات التاريخية المستنتجة إلى حقيقة حالية. لم تُضف migration أو اعتماديات أو قيم أكاديمية وهمية.
- ملاحظة تنسيق: تحديث `PROJECT_MEMORY.md` و`AI_CHANGELOG.md` الخاص بهذا القرار مؤجل لأنهما محجوزان حاليًا لـPR #193؛ لا يتم كسر الحجز.

## مهمة موازية غير متقاطعة — NAVIXA Academic Memory Foundation

- الوكيل: ChatGPT.
- المهمة: بناء طبقة المخزون الأكاديمي التاريخي التي تفصل الحقيقة الحالية عن التاريخ والتوقع، وتحافظ على سياق الجامعة/المنطقة/الفرع/البرنامج/الخطة/السمستر ومصدر كل معلومة وزمن التحقق منها.
- الحالة: معتمدة من المستخدم وبدأت الآن كامتداد آمن لـDraft PR #212؛ لا دمج ولا نشر أثناء التنفيذ.
- الملفات المحجوزة: `app/education/academic-memory.ts`, `tests/academic-memory.test.ts`, وملفات PR #212 الأكاديمية فقط إذا احتاج الربط بينها؛ لا تشمل ملفات UI System أو `package.json`.
- القيود: الاسم أو رمز المقرر وحدهما لا يسمحان بدمج سجلين؛ اختلاف المنطقة/الفرع/الخطة يبقى منفصلًا. الاستنتاج التاريخي يبقى Prediction ولا يتحول تلقائيًا إلى Fact مهما تكرر، وكل Fact يحتاج مصدرًا وزمن تحقق.

## آخر ما اكتمل

- اكتمل أساس Academic Section Linkage على Draft PR #212 دون نشر: نموذج Course Offering ومكوّنات محاضرة/معمل/تمارين، خيارات تسجيل كاملة، Evidence للرابط، رفض الخلط بين مكونات غير مترابطة، نمط دراسة مستقل لكل meeting، وUID/تنبيه فريد لكل meeting، مع نجاح جميع بوابات التحقق والأمن.
...
- تنفيذ Academic Memory Foundation على PR #212: هوية سياق صارمة، Evidence للمصدر، Current Fact / Historical Observation / Prediction كطبقات منفصلة، وقواعد تمنع التوريث عبر منطقة/فرع/خطة مختلفة.
- بعد نجاح التحقق ومراجعة المستخدم فقط: تقرير هل المرحلة التالية تحتاج تخزين D1 دائمًا أو استيراد مصادر الجامعة أولًا، دون القفز إلى Migration غير لازمة.
