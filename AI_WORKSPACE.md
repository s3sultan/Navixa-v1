...
- الحالة: حجز وبدء التنفيذ على فرع متفرع من Academic Structure + Memory، مع Pull Request متراص مستقل؛ الإنتاج غير متأثر.
- الملفات المحجوزة: `app/education/academic-ingestion.ts`, `tests/academic-ingestion.test.ts`, `tests/academic-section-linkage.test.ts` للتوصيل إلى test suite فقط.
- القيود: `institutionId` إلزامي ولا يجوز استنتاج الجامعة من اسم المقرر أو رمزه؛ المصدر الرسمي لا يُقبل كـofficial إلا إذا طابق نطاق/معرّف مصدر مسجل للجامعة؛ المنطقة/الفرع/الخطة غير المعروفة تبقى غير محلولة ولا تُخمن؛ لا D1 ولا migration ولا اعتماديات ولا UI في هذه المرحلة.
...
- تنفيذ Multi-University Source Ingestion Contract كطبقة تحقق قبل D1: تعريف سجل الجامعات ومصادرها، توثيق provenance، وفصل `accept / review / reject` مع منع أي خلط بين المؤسسات أو تخمين للفرع/الخطة.
- انتظار مراجعة المستخدم لـDraft PR #212 قبل أي دمج أو نشر.
